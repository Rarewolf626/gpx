    ...
    foreach ($resortMetas as $rm) {
        ...
        if ($rmArr = json_decode($rm->meta_value, true)) {
            ...
            if ($rmk === 'ResortFeeSettings') {
                $prop->$rmk = json_decode($rm->meta_value, true);
                continue;
            }
...
function get_property_details_checkout($cid, $ccid = '', $ocid = '', $checkoutcid = '')
{
    ...
    if (!empty($rows)) {
        ...
        foreach ($rows as $row) {
            ...
            $actIndPrice[$book]['WeekPrice'] = $prop->Price ?? 0.00;
            ...
            //owner credit coupon
            $subtotal += max(0.00, round((float)$finalPrice, 2));
            ...
            $prop->ResortFeeSettings['total'] = 0.00;
            if (($prop->ResortFeeSettings['enabled'] ?? false) && ($prop->ResortFeeSettings['fee'] ?? 0) > 0) {
                $prop->ResortFeeSettings['total'] = ($prop->ResortFeeSettings['frequency'] === 'daily') ? ($prop->ResortFeeSettings['fee'] * 7) : $prop->ResortFeeSettings['fee'];
                $resortFee += $prop->ResortFeeSettings['total'];
            }
            $prop->unitPrice = $spOut[$book] ?? $prop->WeekPrice;
            $prop->subtotal = $prop->unitPrice + $prop->ResortFeeSettings['total'] + ($indGuestFeeAmount[$book] ?? 0) + ($indUpgrade[$book] ?? 0) + ($indCPOFee[$book] ?? 0);
        } //end each property in cart
        $finalPrice = number_format($finalPrice, 2);
        if ($finalPrice <= 0) {
            $finalPrice = 0;
        }
        if (!isset($subtotal)) $subtotal = $finalPrice;
        $ppSum = array_sum($pp);
