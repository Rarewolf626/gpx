...
import TransactionCancellationDetails from './TransactionCancellationDetails.vue'
import TransactionGuest from './TransactionGuest.vue'
...
const columns = ref([
    ...
    {key: 'user', label: 'Member Number', enabled: true},
    {key: 'member', label: 'Member Name', enabled: true},
    ...
    {key: 'room', label: 'Room Type', enabled: true},
...
const load = async () => {
    ...
    busy.value = true
    try {
        const response = await axios.get('/gpxadmin/transactions/search/', {params: search.value})
...
watchDebounced(
    () => [
        ...
        search.value.discount, search.value.coupon, search.value.occoupon, search.value.ocdiscount,
    ],
    (value) => {
        searchUpdate()
    },
