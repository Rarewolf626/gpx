    ...
    function addRegions() {
        ...
        foreach ( $countries as $countryXML ) {
            ...
            if ( $wpdb->update( 'wp_gpxCategory',
                ...
                $wpdb->insert('wp_gpxCategory', ['country'=>$country->ItemDescription, 'newCountryID'=>$country->ItemID, 'search_name' => gpx_search_string($country->ItemDescription)]);
            }
  ...
  * */
    ...
    function DAEPayAndCompleteBooking( $post ) {
        ...
        if ( $bookingDisabledActive == '1' ) // this is disabled then don't do anything else
        {
            if ( is_user_logged_in() ) {
                ...
                if ( $role[0] == 'gpx_member' ) {
                    ...
                    $output                 = [ 'ReturnMessage' => $bookingDisabledMessage ];
        ...
        foreach ( $carts as $cart ) {
            ...
            $sProps = get_property_details_checkout( $cartData->user,
                $cartData->propertyID,
                $cartData->user,
                $cartData->user );
            ...
            if ( ! isset( $charged ) ) {
                ...
                $paymentDetails = $shift4->shift_sale( $i4goToken,
                    $fullPriceForPayment,
                    $totalTaxCharged,
                    $paymentRef,
                    $cartData->user );
                    ...
                    if ( $paymentDetailsArr['result'][0]['error']['primaryCode'] == 9961 ) {
                        ...
                        if ( $paymentDetailsArr['result'][0]['error']['primaryCode'] == 9815 ) {
                            ...
                            $wpdb->update( 'wp_payments',
                                [ 'i4go_responsetext' => json_encode( $paymentDetailsArr['result'][0]['error'] ) ],
            ...
            $ims = [
                ...
                'Booking' => [
                    'CreditWeekID'      => $creditweekID,
                    'WeekEndpointID'    => $prop->WeekEndpointID ?? null,
                    'WeekID'            => $prop->weekId ?? null,
                    'GuestFirstName'    => str_replace( " &", ",", $cartData->FirstName1 ),
                    'GuestLastName'     => str_replace( ' &', ',', $cartData->LastName1 ),
                    'GuestEmailAddress' => $email,
                    'Adults'            => $adults,
                    ...
                    'DAEMemberNo'       => $usermeta->DAEMemberNo ?? null,
                    'ResortID'          => $prop->ResortID ?? null,
                    'CurrencyCode'      => $prop->Currency ?? null,
                    'GuestAddress'      => $cartData->Address1 ?? null,
                    'GuestTown'         => $cartData->Address3 ?? null,
                    'GuestState'        => $cartData->Address4 ?? null,
                    'GuestPostCode'     => $cartData->PostCode ?? null,
                    'GuestPhone'        => $cartData->HomePhone ?? null,
                    'GuestCountry'      => $cartData->Address5 ?? null,
                ],
            ];
            ...
            if ( isset( $output['ReturnCode'] ) && in_array( $output['ReturnCode'], $bookingSuccessCodes ) ) {
                ...
                if ( $prop->WeekType == 'ExchangeWeek' || $prop->WeekType == 'Exchange Week' ) {
                    ...
                    //add the credit used
                    $sql = $wpdb->prepare( "UPDATE wp_credit SET credit_used = credit_used + 1 WHERE id=%s",
                        $depositID );
                    $wpdb->query( $sql );
