...
class MysqliConnection extends Connection implements ConnectionInterface {
    ...
    public function select( $query, $bindings = [], $useRead = true ) {
        return $this->run( $query, $bindings, function ( $query, $bindings ) use ( $useRead ) {
            ...
            $statement = $this->prepare($this->getMySqliForSelect( $useRead ), $query);
    ...
    public function cursor( $query, $bindings = [], $useReadPdo = true ) {
        $statement = $this->run( $query, $bindings, function ( $query, $bindings ) use ( $useReadPdo ) {
            ...
            $query = $this->buildSql( $query, $this->prepareBindings( $bindings ) );
            $statement = $this->prepare($this->getMySqliForSelect( $useReadPdo ), $query);
     ...
     */
    protected function prepare( mysqli $mysqli, $query ) {
        $statement    = $mysqli->prepare( $query );
        if ( $statement === false ) {
            throw new \mysqli_sql_exception( $mysqli->error, $mysqli->errno );
        }
        return $this->prepared2($statement);
    }
